...
import { Box, Typography, Card, CardContent, TextField, Button, Grid, Alert } from '@mui/material';
...
const BookingRoom: React.FC = () => {
    ...
    useEffect(() => {
        const checkSession = async () => {
            try {
                const response = await axios.get('http://localhost:8080/users/session', { withCredentials: true });
    ...
    const handleReservation = async (e: React.FormEvent) => {
        ...
        if (numGuests > room.maxGuests) {
            setMessage(`The number of guests exceeds the maximum allowed (${room.maxGuests}).`);
            return;
        }
        ...
        try {
            ...
            if (response.status === 400) {
                ...
                setTimeout(() => navigate('/bookings/reserve', { replace: true }), 2000);
            } else {
